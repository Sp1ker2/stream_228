...
class ScreenRecorder:
    def __init__(self, server_ip="195.133.17.131", upload_interval=60):
        ...
        # Setup output directory per machine
    ...
    def stop_recording(self):
        ...
        if self.current_video_file and self.current_video_file.exists():
            ...
            # Try to transcode to H.264 for browser compatibility if ffmpeg is available
            self._maybe_transcode_to_h264(self.current_video_file)
            return self.current_video_file
        return None

    def _maybe_transcode_to_h264(self, video_path: Path) -> None:
        """If ffmpeg is available, transcode the MP4 to H.264 + faststart for browser playback."""
        ffmpeg = shutil.which('ffmpeg')
        if not ffmpeg:
            logger.info("ffmpeg not found; skipping H.264 transcode")
            return
        try:
            ...
            logger.info("H.264 transcode complete")
        except Exception as e:
            logger.warning(f"Failed to transcode with ffmpeg: {e}")
